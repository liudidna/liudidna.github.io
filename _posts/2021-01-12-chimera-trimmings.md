---
layout: page
title:  "Trimming settings for UCSF Chimera"
date: 2021-01-12
tags: research chimera pdb
---
<a href="#top_title">
...
This is for reference. The original file is from [olibclarke](https://github.com/olibclarke/chimera-trimmings).
More info on [Chimera alias](https://www.cgl.ucsf.edu/chimera/current/docs/UsersGuide/midas/alias.html).
More info on [Chimera select](https://www.cgl.ucsf.edu/chimera/docs/UsersGuide/midas/select.html).
More info on [Chimera Accelerators](https://www.cgl.ucsf.edu/chimera/docs/ContributedSoftware/accelerators/alist.html).
More info on [Chimera volume](https://www.cgl.ucsf.edu/chimera/docs/UsersGuide/midas/volume.html).
...
alias ^rainbow_magma_trunc rainbow "#6B116F","#81176D","#981D69","#B02363","#C92D59","#E03B50","#ED504A","#F66B4D","#FA8657","#FBA368","#FBC17D"

# $1 allows for chain, model designation
alias ^rainbow_ryb rainbow $1 "#2F7AB4","#ACD9E8","#FFFFC2","#FCAF68","#D62328"
...
alias ^ao activate_only
alias activate_all select all
alias ^aa activate_all
...
alias ^local_diffmap_new savepos tmp; ~sel; ac mc; vop zone $2 sel $3 modelid 10000 minimalbounds true; vop zone $1 sel $3 modelid 10001 minimalbounds true; fitmap #10001 #10000; vop subtract #10001 #10000 modelid 10002 minrms true; split_diff_map #10002 #10003; volume #10003 step 1; close #10000,10001,10002; close sel; reset tmp 

alias ^local_correlation_aligned savepos tmp; ~sel; ac mc; vop zone $2 sel $3 modelid 10000 minimalbounds true; vop zone $1 sel $3 modelid 10001 minimalbounds true; fitmap #10001 #10000; vop localCorrelation #10001 #10000 modelid 10002; close #10000,10001; close sel; reset tmp
```
...
# Sets map to x*RMS
# Usage: map_sig #map_id rms
alias ^map_sig volume $1 sdlevel $2
alias ^sig map_sig

alias ^diffmap_sig volume $1 rmsLevel -$2 color #da1200000000 rmsLevel $2 color #0000bda00000 capFaces false
alias ^dsig diffmap_sig

#Set diffmap to absolute +/- levels - useful if map has been sigma scaled and then cropped.
alias ^diffmap_abs volume $1 level -$2 color #da1200000000 level $2 color #0000bda00000 capFaces false
